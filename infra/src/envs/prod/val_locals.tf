locals {
  ...
  ecs_task_observability_name                 = "${var.env}-observability-task"                         # ecs observabilityタスク名
  ...
  cloudwatch_ecr_observability_log_group_name = "/aws/ecs/${var.env}/${var.project_name}/observability" # observabilityコンテナ用cloudwatchロググループ名
